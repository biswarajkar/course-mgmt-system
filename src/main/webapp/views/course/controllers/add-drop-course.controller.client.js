(function () {
    ...
    function AddDropCourseController(UserService, CourseService, $location, $routeParams) {
        var viewModel = this;
                	            		...
                	            		function (response) {
                	            			...
                	            			if (allCourses) {
	                	            			...
	                	            			for (var i = 0; i < allCourses.length; i++) {
	                	            				if (!courseIds.includes(allCourses[i].courseId)) {
	                	            					remCourses.push(allCourses[i]);
	                	            				}
	                	            			}
        ...
        function join(courseId) {
        	var promise = CourseService.assignCourse(courseId, viewModel.params.personId, viewModel.params.roleType);
        ...
        function drop(courseId) {
        	var promise = CourseService.dropCourse(courseId, viewModel.params.personId, viewModel.params.roleType);
        	promise.then(function successCallback() {
        		init();
        	}, function errorCallback(response) {

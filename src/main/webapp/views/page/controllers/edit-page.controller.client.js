(function () {
    ...
    function EditPageController(UserService, CourseService, PageService, $location, $routeParams) {
        var viewModel = this;
            ...
            promiseCourse.then(
        	    function (response) {
                    ...
                    if (course != undefined) {
                        ...
                        viewModel.errorMessage = "Error while loading course by ID:" + courseId;
                    }
                    
                    var promisePage = PageService.findPageById(courseId, pageId);
                    ...
                    promisePage.then(
                    	function (page) {
                    		...
                    		if (page != undefined) {
                    			...
                    			viewModel.errorMessage = "Error while loading page by ID:" + pageId;
                    		}
                    	}
                    );
                }	
        ...
        function update(page) {
            ...
            var promise = PageService.updateByPageId(page.pageId, page.name, page.tooltipDescription);
            promise.then(function successCallback(response) {
                page = response.data;
                if (page) {
                    $location.url(viewModel.user.roleType + "/" + viewModel.user.personId + "/course/" + viewModel.course.courseId + "/page");
                } else {
                    init();
